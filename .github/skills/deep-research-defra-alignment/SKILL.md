...
description: "Do thorough, risk-scoped research in the open and align findings to the DEFRA standards precedence (DEFRA > GDS > community) for the MMO FES PDF Service. Use for the single, risk-scoped Research (§4.2) stage of the working framework — validating APIs, libraries, patterns, security and policy against DEFRA/GDS and framework guidance, and citing sources before a plan is approved or implemented."
argument-hint: "e.g. 'validate the PDFKit tagged-PDF structure approach the planner flagged' or 'research @azure/storage-blob PassThrough upload patterns'"
license: OGL-UK-3.0
metadata:
  author: mmo-fes
  version: "1.0"
...
- **Planner - PDF Service** runs this single research pass for **Complex** work and cites sources directly
  in its plan.
- **Developer - PDF Service** runs this same single pass for **Standard** work (or when invoked without a
  plan) as its own Research (§4.2) stage.
...
correctness** (PDF generation/parsing correctness, QR/`SERVICE_URL` encoding, document-number mapping),
**stream handling** (stream-based Azure Blob uploads, PassThrough, no disk I/O), **external integrations**
(Azure Blob Storage, SAS tokens), the **`src/index.js` public API surface**, or a **version-sensitive
API/library** (the locked PDFKit 0.15.1, muhammara, qr-image, `@azure/storage-blob`). A cosmetic or
...
4. **Community best practice** — OWASP Secure Coding Practices, 12-factor, widely-adopted Node.js/JavaScript patterns
...
- Search **authoritative, current** sources: DEFRA & GOV.UK standards and service manuals, OWASP, and the
  library's own docs (PDFKit, muhammara, `@azure/storage-blob`, qr-image). Prefer primary sources over blog
  posts.
- **Confirm currency:** check the API/pattern is supported on the current Node.js LTS and is not deprecated.
  Note version availability and any migration since — and remember PDFKit is **pinned at 0.15.1**, so
  validate against that version, not the latest.
...
- [ ] **Encrypt in transit** — HTTPS/TLS only to Azure Blob Storage and external services; no plain HTTP.
- [ ] **No secrets in code** — configuration and credentials from environment/config (`src/config.js`) only.
- [ ] **No PII in logs** — names, addresses, emails, phone numbers, NI numbers, bank details, tokens.
- [ ] **Structured logging** — bracketed context tags and correlation IDs at appropriate levels.
- [ ] **Data correctness** — PDF generation/parsing, QR/`SERVICE_URL` encoding and document-number mapping
      preserve integrity; uploads stay stream-based (PassThrough, no disk I/O).
- [ ] **Testing** — change is testable and keeps tiered coverage (≥90% global, ≥95% core logic, 100%
      error-handling/security paths) via `npm test` (and `npm run test:integration` where Azure Blob
      integration is affected), without dropping below the SonarCloud baseline. This repo has **no**
      `npm run lint` or `npm run build` step — do not reference either.
- [ ] **Dependencies** — widely used, actively maintained, current Node.js LTS; **PDFKit stays locked at
      0.15.1 (no upgrade without a full regression)**; no duplicate PDF library, storage SDK or date library.
- [ ] **Public API** — the `src/index.js` public surface is only extended for a genuinely new public method;
      signature changes are flagged.
...
**blocked**); send **revise/blocked** items back to the **Planner - PDF Service** rather than fixing the plan
yourself. Respect the framework's **3-iteration cap** on plan → validate → approve → implement; if a point is
...
- This skill informs decisions only; it does **not** edit code, run tests, or grant approval.
